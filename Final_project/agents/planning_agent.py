...
class PlanningAgent(Agent):
    ...
    def __init__(self, collection):
        ...
        self.ensemble = Ensemble(collection)
    ...
    def run(self, deal: Deal) -> Opportunity:
        ...
        return Opportunity(deal=deal, estimate=estimate, discount=discount)#creating obj of the class with no init and just attributes but still wel be an onject of atha class
    ...
    def plan(self, memory: List[str] = []) -> Optional[Opportunity]:
        ...
        if selection:
            opportunities = [self.run(deal) for deal in selection.deals[:2]]
